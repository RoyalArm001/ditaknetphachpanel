...
const exportValue=(r,k)=>k==='status'?Domain.statuses[r[k]]:k==='service'?Domain.services[r[k]].label:r[k];
  ...
  const server=http.createServer(async(req,res)=>{
    ...
    try{
      ...
      if(authRequired&&url.pathname.startsWith('/api/')){
        if(['POST','PUT','DELETE'].includes(req.method)&&req.headers.origin&&req.headers.origin!==`${cloud?'https':'http'}://${req.headers.host}`)return json(res,403,{error:'Օտար էջից փոփոխությունն արգելված է'});
        if(url.pathname==='/api/auth/pin'&&req.method==='POST'&&pinAuth){
          if(!req.headers['content-type']?.startsWith('application/json'))return json(res,415,{error:'Պահանջվում է JSON'});
          const body=await readBody(req),user=await pinAuth.login(req,res,body.pin);
          if(user?.limited){res.setHeader('Retry-After','900');return json(res,429,{error:'Շատ փորձեր։ Կրկին փորձեք 15 րոպեից։'});}
          return user?json(res,200,{user}):json(res,401,{error:'PIN կոդը սխալ է'});
        }
        if(url.pathname==='/api/auth/login'&&req.method==='POST'){
          if(!req.headers['content-type']?.startsWith('application/json'))return json(res,415,{error:'Պահանջվում է JSON'});
          const credentials=await readBody(req);const user=await auth.login(res,credentials.email,credentials.password);
          return user?json(res,200,{user}):json(res,401,{error:'Մուտքը չհաջողվեց կամ RackMap-ի թույլտվությունը բացակայում է'});
        }
        ...
        if(!user)return json(res,401,{error:'Մուտք գործեք Ditaknet-ի ձեր հաշվով'});
      ...
      if(req.method==='POST'&&url.pathname==='/api/companies'){
        let body;try{body=await readBody(req);}catch(e){return json(res,e.code||400,{error:'Հարցման ձևաչափը սխալ է'});}
        const name=typeof body.name==='string'?body.name.trim():'';
        if(!name||name.length>200||!Number.isInteger(body.floorCount)||body.floorCount<1||body.floorCount>200)return json(res,400,{error:'Նշեք ընկերության անունը և 1–200 հարկ'});
        if((await listCompanies()).some(x=>x.name.toLocaleLowerCase()===name.toLocaleLowerCase()))return json(res,400,{error:'Այս անունով ընկերություն արդեն կա'});
        const state={...Domain.empty(),company:name,floors:Array.from({length:body.floorCount},(_,i)=>({id:randomUUID(),name:`${i+1}-րդ հարկ`,racks:[]}))};
      ...
      if(scoped&&!current)return json(res,404,{error:'Ընկերությունը չի գտնվել'});
      ...
      if(req.method==='PUT'&&url.pathname==='/api/state'){
        ...
        if(body.state.company&&(await listCompanies()).some(x=>x.id!==companyId&&x.name.toLocaleLowerCase()===body.state.company.trim().toLocaleLowerCase()))return json(res,400,{error:'Այս անունով ընկերություն արդեն կա'});
        const result=await store.save(companyId,body.state,body.revision);
        return result.conflict?json(res,409,{error:'Տվյալները փոփոխվել են այլ աշխատակցի կողմից։ Թարմացրեք էջը։',revision:result.revision}):json(res,200,result);
      }
      ...
      if(req.method==='GET'&&url.pathname.startsWith('/api/history/')){
        ...
        return h?json(res,200,h):json(res,404,{error:'Տարբերակը չի գտնվել'});
      }
      ...
      if(req.method==='GET'&&['/api/export.xlsx','/api/export.pdf'].includes(url.pathname)){
        ...
        if(url.pathname.endsWith('xlsx')){
          ...
          const inventory=book.addWorksheet('Ռաքեր և սարքեր');inventory.columns=[{header:'Հարկ',key:'floor',width:24},{header:'Ռաք',key:'rack',width:24},{header:'Ռաք U',key:'u',width:12},{header:'Սարք',key:'device',width:24},{header:'U դիրք',key:'pos',width:12},{header:'Բարձրություն U',key:'height',width:18},{header:'Պորտեր',key:'ports',width:12}];
        ...
        pdf.fontSize(12).fillColor('#333333').text('Ցանցային միացումների հաշվետվություն');pdf.fontSize(9).text(new Date().toLocaleString('hy-AM'));pdf.moveDown();
        const filterLabels=[url.searchParams.get('query'),state.floors.find(x=>x.id===url.searchParams.get('floor'))?.name,Domain.devices(state).find(x=>x.r.id===url.searchParams.get('rack'))?.r.name,Domain.statuses[url.searchParams.get('status')]].filter(Boolean);
        if(filterLabels.length)pdf.text(filterLabels.join(' / '));pdf.text(`Պորտերի քանակ՝ ${rows.length}`).moveDown();
        for(const row of rows){
          const lines=[`${row.floor} / ${row.rack} / ${row.device} / ${row.port}`,`Վիճակ՝ ${Domain.statuses[row.status]}    Մալուխ՝ ${row.cable||'—'}`,`Նշանակություն՝ ${Domain.services[row.service].label}    VLAN՝ ${row.vlan||'—'}`,`Տեղադրություն՝ ${[row.destination,row.room,row.door,row.side].filter(Boolean).join(' / ')||'—'}`,`Կապ՝ ${row.connection||'—'}`,row.notes?`Նշումներ՝ ${row.notes}`:''].filter(Boolean);
        ...
        if(!rows.length)pdf.text('Ընտրված ֆիլտրերով պորտեր չկան։');
      ...
      const files={'/':'index.html','/index.html':'index.html','/app.js':'app.js','/domain.js':'domain.js','/rack3d.js':'rack3d.js','/styles.css':'styles.css','/manifest.webmanifest':'manifest.webmanifest','/sw.js':'sw.js','/pwa.js':'pwa.js','/icon-192.png':'icon-192.png','/icon-512.png':'icon-512.png'};
    ...
    }catch(e){console.error('RackMap request failed:',e.code||e.name);if(!res.headersSent)json(res,e.code==='23505'?400:500,{error:'Չհաջողվեց կատարել գործողությունը'});else res.end();}
  });
