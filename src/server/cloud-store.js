...
function createPool(env=process.env){
  ...
  const ca=env.POSTGRES_SSL_CA_PEM?.replace(/\\n/g,'\n')||(env.POSTGRES_SSL_CA?fs.readFileSync(env.POSTGRES_SSL_CA,'utf8'):/\.supabase\.com$/.test(url.hostname)?fs.readFileSync(require('node:path').join(__dirname,'../../assets/certificates/supabase-ca.crt'),'utf8'):undefined);
