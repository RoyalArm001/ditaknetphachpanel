...
const ensureNetworks=s=>{if(!Array.isArray(s.networks))s.networks=[];return s.networks;};
const networkLabel=n=>`${n.name} · VLAN ${n.vlan}${n.ip?' · '+n.ip:''}`;
const deviceChoices=()=>[['',tr('Առանց ռաքի սարք')],...D.devices(state).map(x=>[x.d.id,`${x.f.name} / ${x.r.name} / ${x.d.name}`])];
...
function styleFields(source={},open=false){
  const rows=(kind,entries)=>entries.map(([key,label])=>{
    const suffix=key||'none',custom=source[kind+'Labels']?.[key]||'';
    const color=kind==='service'?D.serviceColor(source,key):D.statusColor(source,key);
    return `<div class="project-style-row">${input(`style-${kind}-label-${suffix}`,label,custom,'text',`maxlength="80" placeholder="${esc(label)}"`)}${input(`style-${kind}-color-${suffix}`,tr('Գույն'),color,'color')}</div>`;
  }).join('');
  return tr`<details class="project-style" ${open?'open':''}><summary>Նախագծի անվանումներ և գույներ</summary><p class="hint">Ընտրեք ձեր նշանակությունների և պորտերի վիճակների անվանումներն ու գույները։ Դատարկ անվանումը պահպանում է թարգմանվող սկզբնական տարբերակը։</p><fieldset><legend>Պորտերի նշանակություններ</legend>${rows('service',Object.entries(D.services).map(([key,x])=>[key,x.label]))}</fieldset><fieldset><legend>Պորտերի վիճակներ</legend>${rows('status',Object.entries(D.statuses))}</fieldset></details>`;
}
function readStyle(fd){
  const result={serviceLabels:{},serviceColors:{},statusLabels:{},statusColors:{}};
  for(const [kind,keys]of [['service',Object.keys(D.services)],['status',Object.keys(D.statuses)]])for(const key of keys){
    const suffix=key||'none',label=String(fd.get(`style-${kind}-label-${suffix}`)||'').trim();
    if(label)result[kind+'Labels'][key]=label;
    const color=fd.get(`style-${kind}-color-${suffix}`);if(color)result[kind+'Colors'][key]=color;
  }
...
function recoverAccountDialog(){
  modal(tr('Վերականգնել իմ ֆայլը'),tr`<p class="hint">Մուտքագրեք միայն PIN կոդը։ Անձնական PIN-ը կբացի ձեր հաշիվը, իսկ թիմային PIN-ը՝ ընդհանուր բազան։</p>${input('pin',tr('PIN կոդ'),'','password','required inputmode="numeric" pattern="[0-9]{8,12}" minlength="8" maxlength="12" autocomplete="one-time-code"')}`,async fd=>{
    ...
    storageMode='account';onboardingChoice='account';onboardingVisible=false;accountUserId=result.user.id;accountReadOnly=false;
    rememberWorkspace();
    activeCompanyId='default';state=D.empty();dirty=false;conflict=false;portDraftDirty=false;portDraft=null;selectedPortId='';
...
function renderOverview(){const rs=racks(),ds=D.devices(state),all=D.rows(state),used=all.filter(x=>x.status==='used').length,fault=all.filter(x=>x.status==='fault').length;
  const nets=D.networks(state),hostCount=nets.reduce((n,x)=>n+x.hosts.length,0);
  $('#content').innerHTML=header(state.company,tr('Շենքի ցանցային ենթակառուցվածքը՝ մեկ տեղում'),button(tr('＋ Ավելացնել հարկ'),'floor','','primary'))+tr`<div class="stats">${[[tr('Հարկեր'),state.floors.length,tr('Շենքի հարկերի քանակը')],[tr('Ռաքեր'),rs.length,tr`${ds.length} տեղադրված սարք`],[tr('Միացված պորտեր'),used,tr`${all.length} պորտից`],[tr('Ցանցեր'),nets.length,tr`${hostCount} սարքի IP`]].map(([l,n,h])=>`<div class="stat"><label>${l}</label><strong>${n}</strong><small>${h}</small></div>`).join('')}</div><div class="section-head"><h2>Հարկեր և ռաքեր</h2>${legend()}</div>${floorCards()||tr('<div class="empty">Ավելացրեք առաջին հարկը</div>')}${nets.length?`<section class="panel" style="margin-top:22px"><div class="section-head"><h2>${tr('Ցանցերի ինֆորմացիա')}</h2><a class="button small" href="#networks">${tr('Բացել')}</a></div>${nets.map(n=>`<div class="device-item"><div><strong>${esc(n.name)}</strong><small>VLAN ${esc(n.vlan)}${n.ip?' · '+esc(n.ip):''} · ${n.hosts.length} ${tr('սարք')}</small></div></div>`).join('')}</section>`:''}${fault?tr`<section class="panel" style="margin-top:22px"><div class="section-head"><h2>Ստուգման ենթակա պորտեր</h2></div>${resultsTable(all.filter(x=>x.status==='fault').slice(0,6))}</section>`:''}`;
}
...
function networkModal(id=''){
  ...
  modal(network?tr('Խմբագրել VLAN ցանցը'):tr('Նոր VLAN ցանց'),`<div class="form-grid">${input('name',tr('Ցանցի անվանում'),network?.name||'','text','required maxlength="200" placeholder="Տեսախցիկներ"')}${input('vlan',tr('VLAN ID'),network?.vlan||'','number','required min="1" max="4094" step="1" placeholder="10"')}${input('ip',tr('Ցանցի IP կամ CIDR'),network?.ip||'','text','maxlength="200" placeholder="192.168.10.0/24"')}</div><fieldset class="project-style"><legend>${tr('Այս VLAN-ի սարքերի տեսակները')}</legend><p class="hint">${tr('Ընտրեք, թե այս VLAN-ը որ տեսակի սարքերի համար է նախատեսված։')}</p><div class="service-choices">${typeOptions}</div></fieldset><p class="hint">${tr('VLAN ID-ն միայն ցանցի նույնացուցիչ է և ինքնուրույն չի փոխում պորտի նշանակությունը։')}</p>`,fd=>commit(s=>{
    ...
    if(target)Object.assign(target,{name,vlan,ip,deviceTypes});else networks.push({id:uid(),name,vlan,ip,deviceTypes,hosts:[]});
...
function deviceTypesModal(){
  ...
  modal(tr('Սարքերի տեսակներ'),tr`<p class="hint">Ավելացրեք ձեր սարքերի տեսակները։ Դրանք կհայտնվեն «Նոր սարք» ձևի տեսակների ցանկում։ Փաչ պանելն ու սվիչը համակարգային տեսակներ են։</p>${custom.length?`<div class="device-type-list">${custom.map(x=>`<div class="device-item"><strong>${esc(x.name)}</strong>${button(tr('Ջնջել'),'device-type-delete',x.id,'small danger')}</div>`).join('')}`:tr('<p class="hint">Ձեր սեփական տեսակներ դեռ չկան։</p>')}${input('deviceTypeName',tr('Նոր սարքի տեսակի անվանում'),'','text','maxlength="80" placeholder="Օրինակ՝ DVR կամ Access Point"')}`,fd=>commit(s=>{
...
function deviceModal(id,rackId,position){const found=findDevice(id),d=found?.d,r=found?.r||findRack(rackId)?.r;if(!r)return;
  ...
  modal(d?tr('Խմբագրել սարքը'):tr('Նոր սարք'),tr`<div class="form-grid">${input('name',tr('Սարքի անվանում'),d?.name||'','text',tr('required maxlength="200" placeholder="PP-01 կամ SW-01"'))}${select('type',tr('Սարքի տեսակ'),D.deviceTypes(state),d?.type||'panel')}${select('modelType',tr('Սվիչի մոդելի տեսակ'),[['',tr('Ընտրել')],['poe','PoE'],['poe-plus','PoE+'],['none',tr('Առանց PoE')]],d?.modelType||'')}${input('pos',tr('Սկզբնական U դիրք ներքևից'),d?.pos||nextPosition,'number',`required min="1" max="${r.u}"`)}${input('height',tr('Բարձրություն U'),d?.height||1,'number',`required min="1" max="${r.u}"`)}${select('count',tr('Պորտերի քանակ'),[],d?.portList.length||24)}<div class="field"><label for="color">${tr('Փաչ պանելի կամ սվիչի գույնը 3D-ում')}</label><input id="color" name="color" type="color" value="${esc(d?.color||'#397c78')}" required></div>${!d?select('initialStatus',tr('Նոր պորտերի սկզբնական վիճակ'),Object.keys(D.statuses).map(key=>[key,D.statusLabel(state,key)]),'free')+select('initialService',tr('Նոր պորտերի նշանակություն'),Object.keys(D.services).map(key=>[key,D.serviceLabel(state,key)]),''):''}<datalist id="heights"><option value="1"><option value="2"><option value="4"></datalist><datalist id="portCounts"><option value="12"><option value="24"><option value="48"></option></datalist>
  <div class="field full"><p class="hint">${nets.length?tr('Սարքի IP-ն, username-ը և գաղտնաբառը պահվում են ընտրված VLAN ցանցում։'):tr('Նախ Ցանցեր բաժնում ավելացրեք VLAN։')}</p></div>
  ${nets.length?select('hostNetworkId',tr('Ցանց'),[['',tr('Չնշել')],...nets.map(n=>[n.id,networkLabel(n)])],first?.n.id||'')+input('hostIp',tr('Սարքի IP'),first?.h.ip||'','text',tr('maxlength="200" placeholder="192.168.10.2" inputmode="decimal" autocomplete="off"'))+input('hostUsername',tr('Մուտքանուն'),first?.h.username||'','text','maxlength="200" autocomplete="off"')+secretInput('hostPassword',tr('Գաղտնաբառ'),first?.h.password||''):''}</div><p class="form-note">Սվիչի համար ընտրեք PoE, PoE+ կամ առանց PoE։</p>`,fd=>commit(s=>{
    ...
    if(networkId||ip||username||password){
      ...
      const net=ensureNetworks(s).find(x=>x.id===networkId);if(!net)throw new Error(tr('Ցանցը չի գտնվել'));
      const existing=net.hosts.find(h=>h.deviceId===edit.id)||(first&&first.n.id===networkId?net.hosts.find(h=>h.id===first.h.id):null);
      if(existing)Object.assign(existing,{name:edit.name,ip,username,password,deviceId:edit.id});
      else net.hosts.push({id:uid(),name:edit.name,ip,username,password,deviceId:edit.id});
    }
...
function serviceLegend(){return '<div class="service-legend">'+Object.entries(D.services).map(([key,x])=>`<span><i style="background:${D.serviceColor(state,key)}"></i>${esc(D.serviceLabel(state,key))}</span>`).join('')+'</div>';}
function paintPorts(){
...
function renderPortTools(){
  ...
  <form id="portForm">
    ...
    <div class="form-grid">
      ${select('status',tr('Վիճակ'),incoming?[['used',D.statusLabel(state,'used')],['fault',D.statusLabel(state,'fault')]]:Object.keys(D.statuses).map(key=>[key,D.statusLabel(state,key)]),v.status)}
    ...
    <details class="port-extra"><summary>${tr('Այլ տվյալներ')}</summary><div class="form-grid">
      ...
      <div class="field full"><label for="notes">${tr('Լրացուցիչ նշումներ')}</label><textarea id="notes" name="notes" maxlength="2000">${esc(v.notes)}</textarea></div>
    </div></details>
...
function moveDevice(id,direction){
  const found=D.devices(state).find(x=>x.d.id===id);
  if(!found)return;
  const next=found.d.pos+(direction==='up'?1:-1);
  if(next<1||next+found.d.height-1>found.r.u){toast(tr('Սարքը չի կարող դուրս գալ ռաքի սահմաններից'));return;}
  if(found.r.devices.some(d=>d.id!==id&&next<d.pos+d.height&&next+found.d.height>d.pos)){toast(tr('Այս դիրքը զբաղված է այլ սարքով'));return;}
  commit(s=>{const d=D.devices(s).find(x=>x.d.id===id)?.d;if(d)d.pos=next;});
  render();
}
...
async function backup(){const data={application:'RackMap',schema:2,exportedAt:new Date().toISOString(),state,portDraft:portDraftDirty?portDraft:null},format=state.backupFormat||'json';const blob=format==='xlsx'?new Blob([await ProjectFile.encode(data,tr)],{type:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'}):new Blob([JSON.stringify(data,null,2)],{type:'application/json'});download(blob,'MyPatch-'+new Date().toISOString().slice(0,10)+'.'+format);}
...
async function readBackupFile(file){if(file.size>24*1024*1024)throw new Error(tr('Ֆայլը չափազանց մեծ է'));let data;try{data=/\.xlsx$/i.test(file.name)?await ProjectFile.decode(file):JSON.parse(await file.text());}catch{throw new Error(tr('Պահուստային ֆայլը վնասված է կամ չի աջակցվում'));}
...
async function restoreFile(file,{personalWorkspace=false}={}){if(!file)return;const data=await readBackupFile(file);
  ...
  confirmAction(tr('Վերականգնել պատճենը'),tr`Ընկերություն՝ ${next.company}։ Հարկեր՝ ${next.floors.length}։ Ներկայիս տվյալները կփոխարինվեն և կպահվեն պատմության մեջ։`,async()=>{
    ...
    if(!await save())throw new Error(tr('Չհաջողվեց կատարել գործողությունը'));
...
function backupFormatField(value='xlsx'){return select('backupFormat',tr('Պահուստային ֆայլի տեսակ'),[['xlsx','Excel (.xlsx)'],['json','JSON (.json)']],value);}
function newCompany(){
  modal(tr('Նոր ընկերություն'),tr`<div class="form-grid">${input('name',tr('Ընկերության անվանում'),'','text','required maxlength="200"')}${input('floorCount',tr('Հարկերի քանակ'),1,'number','required min="1" max="200"')}</div>${select('backupFormat',tr('Պահուստային ֆայլի տեսակ'),[['xlsx','Excel (.xlsx)'],['json','JSON (.json)']],'xlsx')}${styleFields()}<p class="hint">Կստեղծվի առանձին ընկերություն։ Գործող ընկերության ռաքերն ու կապերը կմնան իրենց տեղում։</p>`,async fd=>{
    if(!await save())throw new Error(tr('Նախ պահպանեք գործող ընկերության փոփոխությունները։'));
    const data=await api('/api/companies',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:fd.get('name'),floorCount:Number(fd.get('floorCount')),style:{...readStyle(fd),backupFormat:fd.get('backupFormat')||'xlsx'}})});
...
const actions={
  ...
  'scene-up':()=>sceneController?.tilt(-.12),'scene-down':()=>sceneController?.tilt(.12),
  ...
  company:()=>modal(tr('Ընկերություն'),input('name',tr('Ընկերության անվանում'),state.company,'text','required maxlength="200"')+backupFormatField(state.backupFormat||'json'),fd=>commit(s=>{s.backupFormat=fd.get('backupFormat');s.company=fd.get('name').trim();if(!s.company)throw new Error(tr('Անվանումը պարտադիր է'));})),
