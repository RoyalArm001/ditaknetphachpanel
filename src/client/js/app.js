...
function renderNetworks(){
  ...
  const typeLabel=type=>D.services[type]?D.serviceLabel(state,type):type;
  const ipText=n=>{const ips=Array.isArray(n.ip)?n.ip:(n.ip?[n.ip]:[]);return ips.length?' · '+ips.map(esc).join(', '):'';};
...
function networkModal(id=''){
  ...
  const typeOptions=Object.keys(D.services).filter(key=>key).map(key=>[key,D.serviceLabel(state,key)]);
  if(network?.name&&!typeOptions.some(([key])=>key===network.name))typeOptions.unshift([network.name,network.name]);
  const ips=Array.isArray(network?.ip)?network.ip:(network?.ip?[network.ip]:['']);
...
const actions={
  ...
  'network-ip-add':()=>{const box=$('#networkIpFields');if(!box)return;const i=box.querySelectorAll('input[name="ip"]').length;box.insertAdjacentHTML('beforeend',`<div class="network-ip-row"><div class="field"><label for="network-ip-${i}">${tr('Ցանցի IP կամ CIDR')} ${i+1}</label><input id="network-ip-${i}" name="ip" type="text" maxlength="200" placeholder="192.168.10.0/24" inputmode="decimal"></div>${button('−','network-ip-remove',String(i),'small danger')}</div>`);},
  'network-ip-remove':index=>{const fields=[...document.querySelectorAll('#networkIpFields .network-ip-row')];if(fields.length>1)fields[Number(index)]?.remove();},
