...
function networkModal(id=''){
  ...
  modal(network?tr('Խմբագրել VLAN ցանցը'):tr('Նոր VLAN ցանց'),`<div class="form-grid">${select('name',tr('Ցանցի տեսակ'),typeOptions,network?.name||'')}${input('vlan',tr('VLAN ID'),network?.vlan||'','number','required min="1" max="4094" step="1" placeholder="10"')}${input('ip',tr('Ցանցի IP կամ CIDR'),network?.ip||'','text','maxlength="200" placeholder="192.168.10.0/24"')}</div><p class="hint">${tr('Ընտրեք, թե այս ցանցը ինչ սարքերի համար է՝ տեսախցիկ, հեռախոս, Wi-Fi և այլն։')}</p><p class="hint">${tr('VLAN ID-ն միայն ցանցի նույնացուցիչ է և ինքնուրույն չի փոխում պորտի նշանակությունը։')}</p>`,fd=>commit(s=>{
