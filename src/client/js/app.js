...
function deviceModal(id,rackId,position){const found=findDevice(id),d=found?.d,r=found?.r||findRack(rackId)?.r;if(!r)return;
  ...
  modal(d?tr('Խմբագրել սարքը'):tr('Նոր սարք'),tr`<div class="form-grid">${input('name',tr('Սարքի անվանում'),d?.name||'','text',tr('required maxlength="200" placeholder="PP-01 կամ SW-01"'))}${select('type',tr('Սարքի տեսակ'),[['panel',tr('Փաչ պանել')],['switch',tr('Սվիչ')]],d?.type||'panel')}${select('modelType',tr('Սվիչի մոդելի տեսակ'),[['',tr('Ընտրել')],['poe','PoE'],['poe-plus','PoE+'],['none',tr('Առանց PoE')]],d?.modelType||'')}${input('pos',tr('Սկզբնական U դիրք ներքևից'),d?.pos||nextPosition,'number',`required min="1" max="${r.u}"`)}${input('height',tr('Բարձրություն U'),d?.height||1,'number',`required min="1" max="${r.u}" list="heights"`)}${select('count',tr('Պորտերի քանակ'),[],d?.portList.length||24)}${input('color',tr('Սարքի գույն'),d?.color||'#397c78','color')}${!d?select('initialStatus',tr('Նոր պորտերի սկզբնական վիճակ'),Object.keys(D.statuses).map(key=>[key,D.statusLabel(state,key)]),'free')+select('initialService',tr('Նոր պորտերի նշանակություն'),Object.keys(D.services).map(key=>[key,D.serviceLabel(state,key)]),''):''}<datalist id="heights"><option value="1"><option value="2"><option value="4"></datalist><datalist id="portCounts"><option value="12"><option value="24"><option value="48"></option></datalist></div><p class="form-note">Սվիչի համար ընտրեք PoE, PoE+ կամ առանց PoE։</p>`,fd=>commit(s=>{
