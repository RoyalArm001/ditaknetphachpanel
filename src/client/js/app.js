...
const networkTypePreference=()=>{try{return localStorage.getItem('rackmap-network-type')||'';}catch{return '';}};
const rememberNetworkType=type=>{try{localStorage.setItem('rackmap-network-type',type);}catch{}};
const networkName=n=>D.hasService(state,n.name)?D.serviceLabel(state,n.name):n.name;
const networkLabel=n=>`${networkName(n)} · VLAN ${n.vlan}${n.ip?' · '+n.ip:''}`;
...
function styleFields(source={},open=false){
  ...
  const content=`<p class="hint">${tr('Ավելացրեք ցանցի տեսակ, խմբագրեք անվանումն ու գույնը կամ ջնջեք չօգտագործվող տեսակը։ Փոփոխությունները կիրառվում են «Պահպանել» սեղմելուց հետո։')}</p><fieldset><legend>${tr('Ցանցերի տեսակներ և պորտերի նշանակություններ')}</legend><div data-service-style-rows>${D.serviceEntries(source).map(([key,x])=>serviceStyleRow(source,key,x.label)).join('')}</div>${button(tr('＋ Ավելացնել ցանցի տեսակ'),'style-service-add','','small')}<p class="hint">${tr('Օգտագործվող տեսակը ջնջելուց առաջ փոխեք այն ցանցերում և պորտերում։')}</p></fieldset><fieldset><legend>${tr('Պորտերի վիճակներ')}</legend>${statuses}</fieldset>`;
  return open?`<div class="project-style">${content}</div>`:`<details class="project-style"><summary>${tr('Նախագծի անվանումներ և գույներ')}</summary>${content}</details>`;
}
...
function networkModal(id=''){
  ...
  const typeOptions=D.serviceEntries(state).filter(([key])=>key).map(([key])=>[key,D.serviceLabel(state,key)]);
  if(network?.name&&!typeOptions.some(([key])=>key===network.name))typeOptions.unshift([network.name,network.name]);
  const rememberedType=networkTypePreference();
  const selectedType=network?.name||(typeOptions.some(([key])=>key===rememberedType)?rememberedType:typeOptions[0]?.[0]||'');
  ...
  modal(network?tr('Խմբագրել VLAN ցանցը'):tr('Նոր VLAN ցանց'),`<div class="form-grid">${select('name',tr('Ցանցի տեսակ'),typeOptions,network?.name||'')}${input('vlan',tr('VLAN ID'),network?.vlan||'','number','required min="1" max="4094" step="1" placeholder="10"')}</div><div id="networkIpFields">${ipFields}</div><p class="hint">${tr('VLAN ID-ն միայն ցանցի նույնացուցիչ է և ինքնուրույն չի փոխում պորտի նշանակությունը։')}</p>`,fd=>commit(s=>{
...
function renderPortTools(){
  ...
  if(selectedPortIds.size){
    const services=D.serviceEntries(state);
  ...
  if(!item||item.r.id!==route().id){
    host.innerHTML=tr`<div class="eyebrow">ԳՈՐԾԻՔՆԵՐ</div><h2>Պորտի խմբագրում</h2><p class="hint">Սեղմեք ռաքի պորտին։ Տվյալները կխմբագրվեն այստեղ՝ ռաքի կողքին։</p><h3 class="link-heading">Կապի կարգավորումներ · գույներ</h3>${serviceLegend()}<p class="hint">Պորտի հիմնական գույնը ցույց է տալիս նշանակությունը։ Փոքր կետը՝ ազատ, զբաղված կամ անսարք վիճակը։</p>`;
    return;
  }
  const {p,d,r}=item;
...
const actions={
  ...
  'style-service-add':(id,el)=>{
    const rows=el.closest('.project-style').querySelector('[data-service-style-rows]');
    if(rows.querySelectorAll('[data-service-style^="custom-"]').length>=100){toast(tr('Առավելագույնը 100 նոր ցանցի տեսակ'));return;}
    const key='custom-'+uid();
    rows.insertAdjacentHTML('beforeend',serviceStyleRow({serviceTypes:[{id:key,name:'',color:'#397c78'}]},key,''));
