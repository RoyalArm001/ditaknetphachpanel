...
test('full UI flow: setup, racks, devices, validation, connections, search, editing and reload',async t=>{
  ...
  const dom=await JSDOM.fromURL(base,{runScripts:'dangerously',resources:'usable',pretendToBeVisual:true,virtualConsole:vc,beforeParse(w){w.localStorage.setItem('rackmap-storage-mode','shared');w.fetch=(url,opts)=>fetch(new URL(url,base),opts);w.structuredClone=structuredClone;w.AbortSignal=AbortSignal;w.matchMedia=()=>({matches:false,addEventListener(){}});w.scrollTo=()=>{};w.confirm=()=>true;w.HTMLDialogElement.prototype.showModal=function(){this.open=true;};w.HTMLDialogElement.prototype.close=function(){this.open=false;};}});
  t.after(async()=>{dom.window.close();await new Promise(r=>server.close(r));});
