...
globalThis.PersonalStore=(()=>{
  ...
  function open(){
    if(!opening)opening=new Promise((resolve,reject)=>{
      ...
      request.onerror=()=>{opening=null;reject(request.error);};request.onblocked=()=>{opening=null;reject(new Error('Փակեք ծրագրի մյուս ներդիրները և կրկին փորձեք'));};
  ...
  async function transaction(work){
    ...
    const done=new Promise((resolve,reject)=>{tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error);tx.onabort=()=>reject(tx.error||new Error('Պահպանումը չհաջողվեց'));});
  ...
  async function request(path,options={}){
    ...
    return transaction(async(companies,history)=>{
      ...
      if(url.pathname==='/api/companies'){
        ...
        const name=body.name?.trim();if(!name||name.length>200||!Number.isInteger(body.floorCount)||body.floorCount<1||body.floorCount>200)throw error('Նշեք անունը և հարկերի քանակը');
        if(all.some(x=>x.state.company.toLowerCase()===name.toLowerCase()))throw error('Այս անունով ընկերություն արդեն կա');
        const state={...RackDomain.empty(),company:name,floors:Array.from({length:body.floorCount},(_,i)=>({id:crypto.randomUUID(),name:`${i+1}-րդ հարկ`,racks:[]}))};
        const row={id:crypto.randomUUID(),revision:0,state};await get(companies.add(row));return row;
      }
      if(url.pathname==='/api/storage')return {directory:'Այս սարքը',database:'Հեռախոսի / բրաուզերի հիշողություն',backups:'JSON պատճեն և վերջին 50 փոփոխությունները',personal:true};
      ...
      if(!current)throw error('Ընկերությունը չի գտնվել',404);
      ...
      if(url.pathname==='/api/state'){
        ...
        if(current.revision!==body.revision)throw error('Տվյալները փոխվել են այլ ներդիրում։ Թարմացրեք էջը։',409);
        if(all.some(x=>x.id!==id&&x.state.company.trim().toLowerCase()===body.state.company.trim().toLowerCase()))throw error('Այս անունով ընկերություն արդեն կա');
      ...
      if(url.pathname.startsWith('/api/history/')){const x=await get(history.get([id,Number(url.pathname.split('/').pop())]));if(!x)throw error('Տարբերակը չի գտնվել',404);return {state:x.body};}
      throw error('Գործողությունը չի գտնվել',404);
    });
  }
  return {request};
})();
