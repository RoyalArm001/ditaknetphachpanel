...
const header=(title,sub,actions='')=>tr`<div class="page-head"><div><div class="eyebrow">RACKMAP / ԱՇԽԱՏԱՆՔԱՅԻՆ ՏԱՐԱԾՔ</div><h1>${esc(title)}</h1><p>${esc(sub)}</p></div><div class="actions">${actions}</div></div>`;
...
if(languageSelect&&globalThis.RackI18n)languageSelect.addEventListener('change',async()=>{
  ...
  try{
    ...
    if(login)renderLogin(method);else if(ready)render();else await init();
