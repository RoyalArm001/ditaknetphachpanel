...
function renderRack(id){const found=findRack(id);if(!found){$('#content').innerHTML=header('Ռաքը չի գտնվել','Ընտրեք ռաքը հարկերի ցանկից')+'<a class="button" href="#floors">← Հարկեր և ռաքեր</a>';return;}const {f,r}=found;
  ...
  $('#content').innerHTML=header(r.name,`${f.name} · ${r.location||'Ռաքի առջևի տեսք'}`,`<a class="button" href="#floors">← Հարկեր</a><a class="button" href="#connections/${r.id}">3D կապեր</a>${button('Խմբագրել ռաքը','rack',r.id)}${button('＋ Սարք','device-new',r.id,'primary')}`)+`<div class="section-head">${legend()}<span class="hint">U1՝ ներքևում · ${r.u}U</span></div><div class="rack-layout"><div class="rack-case"><div class="rack-rails" style="grid-template-rows:repeat(${r.u},${unit}px)">${grid}</div></div><aside class="rack-side" id="rightPanel"><div class="mobile-sheet-head"><strong>Պորտի գործիքներ</strong><button class="button small" data-action="toggle-right" aria-label="Փակել գործիքները">Փակել ×</button></div><section id="portTools" class="panel port-tools" aria-label="Պորտի գործիքներ"></section><details class="panel rack-info"><summary>Ռաքի տվյալներ և լուսանկար</summary><h2>Ռաքի տվյալներ</h2><div class="rack-detail"><div><small>Չափ</small>${r.u}U</div><div><small>Զբաղեցված է</small>${r.devices.reduce((n,d)=>n+d.height,0)}U</div><div><small>Սարքեր</small>${r.devices.length}</div><div><small>Պորտեր</small>${r.devices.reduce((n,d)=>n+d.portList.length,0)}</div></div>${r.photo?`<button class="text-button" data-action="photo" data-id="${r.id}" aria-label="Բացել ռաքի լուսանկարը"><img class="photo" src="${esc(r.photo)}" alt="${esc(r.name)} լուսանկար"></button>`:'<div class="empty" style="padding:18px 0">Լուսանկար չկա</div>'}${button(r.photo?'Փոխել լուսանկարը':'＋ Ավելացնել լուսանկար','photo-upload',r.id,'small')}${r.photo?button('Հեռացնել','photo-remove',r.id,'small'):''}<input type="file" id="photoInput" accept="image/png,image/jpeg,image/webp" hidden></details><section class="panel"><h2>Սարքերի ցանկ</h2>${[...r.devices].sort((a,b)=>b.pos-a.pos).map(d=>`<div class="device-item"><div><strong>${esc(d.name)}</strong><small>U${d.pos} · ${d.height}U · ${esc(d.model||'')}</small></div>${button('Բացել','device-detail',d.id,'small')}</div>`).join('')||'<p class="hint">Սեղմեք ռաքի ազատ U դիրքի վրա՝ սարք ավելացնելու համար։</p>'}</section></aside></div>`;
...
async function init(){
  ...
  try{
    ...
    if(personal()){config={cloud:!localHost,authRequired:false};}
      ...
      if(!response.ok)throw new Error('Թիմային սերվերն այս պահին չի պատասխանում։');
    ...
    const data=await api('/api/state');D.validate(data.state);state=data.state;revision=data.revision;ready=true;status('Պահված է');render();try{if(localStorage.getItem(recoveryKey()))banner('Այս սարքում կա չպահված պատճեն։ Այն կարող եք վերականգնել Կարգավորումներ բաժնից։');}catch{}}catch(e){if(e.code===401&&authRequired){renderLogin();return;}ready=false;document.body.classList.remove('rack-view','login-view');status(personal()?'Սարքի պահպանումն անհասանելի է':'Թիմային կապն ընդհատված է',true);
      $('#content').innerHTML=`<section class="panel setup"><div class="eyebrow">DITAKNET · ՊԱՀՊԱՆՈՒՄ</div><h1>${personal()?'Այս բրաուզերում պահպանումը չբացվեց':'Թիմային բազան այս պահին անհասանելի է'}</h1><p>${personal()?'Ստուգեք բրաուզերի պահպանման թույլտվությունը և ազատ տեղը։ Գործող տվյալները չեն ջնջվել։':'Կարող եք կրկին փորձել կամ աշխատել այս սարքի անձնական բազայով։ Թիմի տվյալները մնում են cloud-ում։'}</p><div class="actions"><button class="button primary" data-action="retry">Կրկին փորձել</button>${personal()?'':'<button class="button" data-action="personal-mode">Բացել անձնական բազան</button>'}</div><details style="margin-top:20px"><summary>Ստուգման մանրամասներ</summary><p>${esc(e.message)}</p></details></section>`;
    }
