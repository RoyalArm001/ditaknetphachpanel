...
function renderLogin(){
  ready=false;document.body.classList.remove('rack-view');$('#dialog').close();
  $('#content').innerHTML='<section class="panel setup"><h1>Ditaknet RackMap</h1><p>Մուտք գործեք Ditaknet-ի ձեր գործող հաշվով։ RackMap-ի համար անհրաժեշտ է աշխատակցի առանձին թույլտվություն։</p><form id="loginForm">'+input('email','Էլ․ փոստ','','email','required autocomplete="username"')+input('password','Գաղտնաբառ','','password','required autocomplete="current-password"')+'<p id="loginError" role="alert"></p><button class="button primary" type="submit">Մուտք գործել</button></form></section>';
  $('#leftPanel').inert=true;$('#rightToggle').hidden=true;
  $('#loginForm').onsubmit=async e=>{e.preventDefault();const b=e.target.querySelector('button');b.disabled=true;try{const fd=new FormData(e.target);await api('/api/auth/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email:fd.get('email'),password:fd.get('password')})});if(dirty||portDraftDirty){ready=true;render();}else await init();}catch(err){if($('#loginError'))$('#loginError').textContent=err.message;}finally{b.disabled=false;}};
}
let panelPrefs={left:true,right:true};
...
function applyPanels(){
  for(const k of ['left','right']){
    ...
    const toggle=$('[data-action="toggle-'+k+'"]');if(toggle)toggle.setAttribute('aria-expanded',String(panelPrefs[k]));
  }
  $('#rightToggle').hidden=route().view!=='rack'||!$('#rightPanel');
}
function togglePanel(k){panelPrefs[k]=!panelPrefs[k];applyPanels();try{localStorage.setItem('rackmap-panels',JSON.stringify(panelPrefs));}catch{}}
...
const viewNames={overview:'Ընդհանուր տեսք',floors:'Հարկեր և ռաքեր',search:'Մալուխներ և որոնում',reports:'Հաշվետվություններ',settings:'Կարգավորումներ',rack:'Ռաքի տեսք',connections:'3D կապեր'};
...
const legend=()=>`<div class="legend"><span>Ազատ</span><span class="used">Զբաղված</span><span class="fault">Անսարք</span></div>`;
...
async function api(url,options){const res=await fetch(companyUrl(url),options);const data=await res.json();if(!res.ok){if(res.status===401&&cloudMode&&url!=='/api/auth/login')renderLogin();const e=new Error(data.error||'Կապի սխալ');e.code=res.status;throw e;}return data;}
...
async function save(){
  ...
  saving=(async()=>{
    while(dirty){
      dirty=false;status('Պահպանվում է…');const body=JSON.stringify({state,revision});
      if(new Blob([body]).size>maxStateBytes){dirty=true;status('Տվյալների չափը գերազանցում է պահպանման սահմանը',true);recovery();banner('Նվազեցրեք լուսանկարների չափը կամ ներբեռնեք JSON պատճենը։');return false;}
...
function commit(fn,redraw=true){if(conflict)throw new Error('Նախ ներբեռնեք ձեր փոփոխությունները և բեռնեք ընդհանուր տարբերակը');const next=structuredClone(state);fn(next);D.validate(next);state=next;scheduleSave();if(redraw)render();}
...
function render(){
  ...
  $('#companyLabel').textContent=state.company||'Նոր ընկերություն';renderCompanySelect();const {view,id}=route();document.body.classList.toggle('rack-view',view==='rack');$('#breadcrumb').textContent=viewNames[view];
...
function renderSetup(){
  $('#content').innerHTML=`<section class="panel setup"><div class="eyebrow">ՆՈՐ ԱՇԽԱՏԱՆՔԱՅԻՆ ՏԱՐԱԾՔ</div><h1>Սկսենք ձեր շենքից</h1><p class="muted">Նշեք ընկերությունն ու հարկերի քանակը։ Հետո ընտրեք այն հարկերը, որտեղ կան ռաքեր։</p><form id="setupForm"><div class="form-grid">${input('company','Ընկերության անվանում','','text','required maxlength="200" placeholder="Ընկերություն"')}${input('count','Հարկերի քանակ',3,'number','required min="1" max="200"')}</div><div class="form-actions"><button class="button primary">Ստեղծել շենքը →</button></div><div id="setupError" class="form-error" hidden></div></form></section>`;
  $('#setupForm').onsubmit=e=>{e.preventDefault();const f=new FormData(e.target);try{commit(s=>{s.company=f.get('company').trim();if(!s.company)throw new Error('Գրեք ընկերության անվանումը');s.floors=Array.from({length:+f.get('count')},(_,i)=>({id:uid(),name:`${i+1}-րդ հարկ`,racks:[]}));});toast('Շենքը ստեղծված է');}catch(err){$('#setupError').hidden=false;$('#setupError').textContent=err.message;}};
}
function floorCards(){return `<div class="floor-grid">${state.floors.map(f=>`<section class="floor-card"><div class="floor-title"><div><h3>${esc(f.name)}</h3><small>${f.racks.length} ռաք · ${f.racks.reduce((n,r)=>n+r.devices.length,0)} սարք</small></div>${button('Խմբագրել','floor',f.id,'small')}</div>${f.racks.map(r=>`<a class="rack-link" href="#rack/${r.id}"><span class="rack-icon">▤</span><span><strong>${esc(r.name)}</strong><small>${r.u}U · ${r.devices.length} սարք ${r.location?'· '+esc(r.location):''}</small></span><span class="arrow">→</span></a>`).join('')||'<div class="empty" style="padding:18px">Այս հարկում ռաք դեռ չկա</div>'}<button class="floor-add" data-action="rack-new" data-id="${f.id}">＋ Ավելացնել ռաք</button></section>`).join('')}</div>`;}
function renderOverview(){const rs=racks(),ds=D.devices(state),all=D.rows(state),used=all.filter(x=>x.status==='used').length,fault=all.filter(x=>x.status==='fault').length;
  $('#content').innerHTML=header(state.company,'Շենքի ցանցային ենթակառուցվածքը՝ մեկ տեղում',button('＋ Ավելացնել հարկ','floor','','primary'))+`<div class="stats">${[['Հարկեր',state.floors.length,'Շենքի հարկերի քանակը'],['Ռաքեր',rs.length,`${ds.length} տեղադրված սարք`],['Միացված պորտեր',used,`${all.length} պորտից`],['Խնդիր ունեցող պորտեր',fault,'Պահանջում են ստուգում']].map(([l,n,h])=>`<div class="stat"><label>${l}</label><strong>${n}</strong><small>${h}</small></div>`).join('')}</div><div class="section-head"><h2>Հարկեր և ռաքեր</h2>${legend()}</div>${floorCards()||'<div class="empty">Ավելացրեք առաջին հարկը</div>'}${fault?`<section class="panel" style="margin-top:22px"><div class="section-head"><h2>Ստուգման ենթակա պորտեր</h2></div>${resultsTable(all.filter(x=>x.status==='fault').slice(0,6))}</section>`:''}`;
}
function renderFloors(){$('#content').innerHTML=header('Հարկեր և ռաքեր',`${state.floors.length} հարկ · ${racks().length} ռաք`,button('＋ Ավելացնել հարկ','floor','','primary'))+floorCards();}
function renderRack(id){const found=findRack(id);if(!found){$('#content').innerHTML=header('Ռաքը չի գտնվել','Ընտրեք ռաքը հարկերի ցանկից')+'<a class="button" href="#floors">← Հարկեր և ռաքեր</a>';return;}const {f,r}=found;
  const rows=D.rows(state),byPort=new Map(rows.map(x=>[x.p.id,x]));const mobile=window.matchMedia('(max-width:760px)').matches;const narrow=window.matchMedia('(max-width:1150px)').matches;const unit=Math.max(mobile?134:88,...r.devices.map(d=>Math.ceil((Math.ceil(d.portList.length/(narrow?12:24))*23+32)/d.height)));let grid='';
  for(let u=r.u;u>=1;u--){const row=r.u-u+1,occupied=r.devices.some(d=>u>=d.pos&&u<d.pos+d.height);grid+=`<div class="rack-tick" style="grid-row:${row};grid-column:1">${u}</div><div class="rack-hole" style="grid-row:${row};grid-column:3">▪</div>`;if(!occupied)grid+=`<button class="rack-blank" data-action="device-new" data-id="${r.id}" data-pos="${u}" style="grid-row:${row};grid-column:2" title="Ավելացնել սարք U${u}">＋ Ազատ դիրք · ${u}U</button>`;}
  for(const d of r.devices){grid+=`<section class="rack-device" style="grid-column:2;grid-row:${r.u-(d.pos+d.height-1)+1} / span ${d.height};--device:${d.color}"><div class="device-head"><button data-action="device-detail" data-id="${d.id}">${esc(d.name)} · ${d.type==='panel'?'Փաչ պանել':'Սվիչ'}</button><small>${d.portList.length} պորտ · ${d.height}U</small></div><div class="mini-ports">${d.portList.map(p=>{const x=byPort.get(p.id);return `<button class="mini-port ${x.status}" data-action="port" data-id="${p.id}" aria-label="${esc(d.name)} պորտ ${p.number} ${D.statuses[x.status]}" title="${esc(`${d.name} / ${p.number} · ${D.statuses[x.status]} · ${x.cable} ${x.room}`)}">${p.number}</button>`;}).join('')}</div></section>`;}
  $('#breadcrumb').textContent=`${f.name} / ${r.name}`;
  $('#content').innerHTML=header(r.name,`${f.name} · ${r.location||'Ռաքի առջևի տեսք'}`,`<a class="button" href="#floors">← Հարկեր</a><a class="button" href="#connections/${r.id}">3D կապեր</a>${button('Խմբագրել ռաքը','rack',r.id)}${button('＋ Սարք','device-new',r.id,'primary')}`)+`<div class="section-head">${legend()}<span class="hint">U1՝ ներքևում · ${r.u}U</span></div><div class="rack-layout"><div class="rack-case"><div class="rack-rails" style="grid-template-rows:repeat(${r.u},${unit}px)">${grid}</div></div><aside class="rack-side" id="rightPanel"><section id="portTools" class="panel port-tools" aria-label="Պորտի գործիքներ"></section><details class="panel rack-info"><summary>Ռաքի տվյալներ և լուսանկար</summary><h2>Ռաքի տվյալներ</h2><div class="rack-detail"><div><small>Չափ</small>${r.u}U</div><div><small>Զբաղեցված է</small>${r.devices.reduce((n,d)=>n+d.height,0)}U</div><div><small>Սարքեր</small>${r.devices.length}</div><div><small>Պորտեր</small>${r.devices.reduce((n,d)=>n+d.portList.length,0)}</div></div>${r.photo?`<button class="text-button" data-action="photo" data-id="${r.id}" aria-label="Բացել ռաքի լուսանկարը"><img class="photo" src="${esc(r.photo)}" alt="${esc(r.name)} լուսանկար"></button>`:'<div class="empty" style="padding:18px 0">Լուսանկար չկա</div>'}${button(r.photo?'Փոխել լուսանկարը':'＋ Ավելացնել լուսանկար','photo-upload',r.id,'small')}${r.photo?button('Հեռացնել','photo-remove',r.id,'small'):''}<input type="file" id="photoInput" accept="image/png,image/jpeg,image/webp" hidden></details><section class="panel"><h2>Սարքերի ցանկ</h2>${[...r.devices].sort((a,b)=>b.pos-a.pos).map(d=>`<div class="device-item"><div><strong>${esc(d.name)}</strong><small>U${d.pos} · ${d.height}U · ${esc(d.model||'')}</small></div>${button('Բացել','device-detail',d.id,'small')}</div>`).join('')||'<p class="hint">Սեղմեք ռաքի ազատ U դիրքի վրա՝ սարք ավելացնելու համար։</p>'}</section></aside></div>`;
  renderPortTools();paintPorts();
}
function renderConnections(id){
  const found=findRack(id);if(!found){$('#content').innerHTML=header('Ռաքը չի գտնվել','Ընտրեք ռաքը հարկերի ցանկից');return;}
  const scene=Rack3D.buildScene(state,id);
  $('#content').innerHTML=header(found.r.name+' · 3D կապեր','Քաշեք պատկերը՝ պտտելու համար, մկնիկի անիվով՝ մոտեցրեք։ Միացում ընտրելիս ընդգծվում են երկու ծայրերը։',`<a class="button" href="#rack/${id}">← Խմբագրել ռաքը</a>`)+`
    <div class="scene-layout"><section class="scene-stage"><div class="scene-toolbar">${button('↶','scene-left')}${button('↷','scene-right')}${button('＋','scene-in')}${button('−','scene-out')}${button('Սկզբնական տեսք','scene-reset')}</div><canvas id="rackCanvas" aria-label="Ռաքի պտտվող 3D մոդել և գրանցված մալուխների կապեր">Միացումների ցանկը հասանելի է կողքի հատվածում։</canvas></section>
    <aside class="panel scene-links"><h2>Միացումներ · ${scene.links.length}</h2>${serviceLegend()}<p class="hint">Պատկերը ցույց է տալիս գրանցված կապերը, ոչ թե մալուխի իրական ֆիզիկական երթուղին։</p>${scene.links.map(l=>`<div class="scene-link" data-link="${l.id}"><button type="button" data-action="scene-select" data-id="${l.id}"><i style="background:${l.color}"></i><strong>${esc(l.cable||'Մալուխ')}</strong><span>${esc(l.label)}</span><small>${esc(D.services[l.service].label)} · VLAN ${esc(l.vlan||'—')}${l.external?' · Այլ ռաք':''}</small></button>${button('Խմբագրել պորտը','port',l.id,'small')}</div>`).join('')||'<p class="empty">Նախ պորտի աջ վահանակում ընտրեք միացված սվիչի պորտը։</p>'}</aside></div>`;
...
function filterControls(){return `<div class="filters"><input aria-label="Որոնում" data-filter="query" value="${esc(filters.query)}" placeholder="Սենյակ, մալուխ, սարք, պորտ կամ նշում…"><select aria-label="Հարկ" data-filter="floor">${opts([['','Բոլոր հարկերը'],...state.floors.map(f=>[f.id,f.name])],filters.floor)}</select><select aria-label="Ռաք" data-filter="rack">${opts([['','Բոլոր ռաքերը'],...racks().filter(x=>!filters.floor||x.f.id===filters.floor).map(x=>[x.r.id,`${x.f.name} / ${x.r.name}`])],filters.rack)}</select><select aria-label="Վիճակ" data-filter="status">${opts([['','Բոլոր վիճակները'],...Object.entries(D.statuses)],filters.status)}</select></div>`;}
function resultsTable(rows){return `<div class="table-wrap"><table><thead><tr><th>Սարք / պորտ</th><th>Վիճակ</th><th>Մալուխ</th><th>Նպատակակետ</th><th>Կապ</th><th></th></tr></thead><tbody>${rows.map(x=>`<tr><td><strong>${esc(x.device)} / ${x.port}</strong><small>${esc(x.floor)} · ${esc(x.rack)}</small></td><td><span class="status ${x.status}">${D.statuses[x.status]}</span></td><td>${esc(x.cable||'—')}</td><td>${esc([x.destination,x.room].filter(Boolean).join(' / ')||'—')}<small>${esc([x.door,x.side].filter(Boolean).join(' / '))}</small></td><td>${esc(x.connection||'—')}</td><td>${button('Բացել','port',x.p.id,'small')}</td></tr>`).join('')}</tbody></table></div>`;}
function renderSearch(view){const report=view==='reports';$('#content').innerHTML=header(report?'Հաշվետվություններ':'Մալուխներ և որոնում',report?'Արտահանումը ներառում է ընտրված ֆիլտրերին համապատասխան բոլոր պորտերը։':'Գտեք պորտը և բացեք մալուխի ամբողջական քարտը։',report?button('↓ Excel','xlsx')+button('↓ PDF','pdf','','primary'):'')+filterControls()+'<div id="results"></div>';renderResults();}
...
function floorModal(id){const f=state.floors.find(x=>x.id===id);modal(f?'Խմբագրել հարկը':'Նոր հարկ',input('name','Հարկի անվանում',f?.name||'','text','required maxlength="200"'),fd=>commit(s=>{if(f)s.floors.find(x=>x.id===id).name=fd.get('name').trim();else s.floors.push({id:uid(),name:fd.get('name').trim(),racks:[]});}),f?button('Ջնջել հարկը','floor-delete',id,'danger'):'');}
function rackModal(id,floorId){const found=findRack(id),r=found?.r;modal(r?'Խմբագրել ռաքը':'Նոր ռաք',`<div class="form-grid">${input('name','Ռաքի անվանում',r?.name||'','text','required maxlength="200"')}${select('floorId','Տեղադրման հարկ',state.floors.map(f=>[f.id,f.name]),found?.f.id||floorId)}${select('preset','Պատրաստի չափ',[['','Հատուկ չափ'],...[6,9,12,24,42].map(n=>[n,n+'U'])],r?.u||24)}${input('u','Ռաքի բարձրություն U',r?.u||24,'number','required min="1" max="60"')}${input('location','Տեղադրության նկարագրություն',r?.location||'','text','maxlength="200" placeholder="Օրինակ՝ միջանցքի աջ կողմ"')}</div>`,fd=>commit(s=>{const target=s.floors.find(f=>f.id===fd.get('floorId'));if(r){let source=s.floors.find(f=>f.racks.some(x=>x.id===id));const edit=source.racks.find(x=>x.id===id);Object.assign(edit,{name:fd.get('name').trim(),u:+fd.get('u'),location:fd.get('location').trim()});if(source.id!==target.id){source.racks=source.racks.filter(x=>x.id!==id);target.racks.push(edit);}}else target.racks.push({id:uid(),name:fd.get('name').trim(),u:+fd.get('u'),location:fd.get('location').trim(),photo:'',devices:[]});}),r?button('Ջնջել ռաքը','rack-delete',id,'danger'):'');$('#preset').onchange=e=>{if(e.target.value)$('#u').value=e.target.value;};}
function deviceModal(id,rackId,position){const found=findDevice(id),d=found?.d,r=found?.r||findRack(rackId)?.r;if(!r)return;
  ...
  modal(d?'Խմբագրել սարքը':'Նոր սարք',`<div class="form-grid">${input('name','Սարքի անվանում',d?.name||'','text','required maxlength="200" placeholder="PP-01 կամ SW-01"')}${select('type','Սարքի տեսակ',[['panel','Փաչ պանել'],['switch','Սվիչ']],d?.type||'panel')}${select('template','Պատրաստի ձևանմուշ',[['','Ընտրել'],['panel-12-1','Փաչ պանել 12 պորտ · 1U'],['panel-24-1','Փաչ պանել 24 պորտ · 1U'],['panel-48-2','Փաչ պանել 48 պորտ · 2U'],['switch-28-1','Սվիչ 28 պորտ · 1U'],['switch-48-1','Սվիչ 48 պորտ · 1U']])}${input('model','Մոդել',d?.model||'','text','maxlength="200"')}${input('pos','Սկզբնական U դիրք ներքևից',d?.pos||nextPosition,'number',`required min="1" max="${r.u}"`)}${input('height','Բարձրություն U',d?.height||1,'number',`required min="1" max="${r.u}" list="heights"`)}${select('count','Պորտերի քանակ',[],d?.portList.length||24)}${input('color','Սարքի գույն',d?.color||'#397c78','color')}<datalist id="heights"><option value="1"><option value="2"><option value="4"></datalist><datalist id="portCounts"><option value="12"><option value="24"><option value="48"></datalist></div><p class="form-note">Փաչ պանել՝ 12, 24 կամ 48 պորտ։ Չափը ճշտեք սարքի մոդելով. 48 պորտանոց պանելը կարող է լինել 1U կամ 2U։ Տեղափոխելիս փոխեք U դիրքը։</p>`,fd=>commit(s=>{
    ...
    const removed=edit.portList.slice(count);if(removed.some(p=>p.status!=='free'||p.cable||p.room||p.notes||p.service||p.vlan||p.switchPortId||D.ports(s).some(x=>x.p.switchPortId===p.id)))throw new Error('Հեռացվող պորտերում կան տվյալներ կամ կապեր։ Նախ մաքրեք դրանք։');
    if(d&&d.type!==fd.get('type')&&edit.portList.some(p=>p.switchPortId||D.ports(s).some(x=>x.p.switchPortId===p.id)))throw new Error('Կապված սարքի տեսակը փոխելուց առաջ անջատեք կապերը։');
  ...
  }),d?button('Ջնջել սարքը','device-delete',id,'danger'):'');
  const setPortChoices=(preferred)=>{
    ...
    $('#count').innerHTML=opts(counts.map(n=>[n,n+' պորտ']),chosen);
  };
...
function deviceDetail(id){const {d,r}=findDevice(id)||{};if(!d)return;const byId=new Map(D.rows(state).map(x=>[x.p.id,x]));modal(d.name,`<p class="port-title">${esc(r.name)} · U${d.pos} · ${d.height}U · ${esc(d.model)} </p>${legend()}<div class="port-overview">${d.portList.map(p=>`<button class="${byId.get(p.id).status}" data-action="port" data-id="${p.id}" title="${D.statuses[byId.get(p.id).status]}">${p.number}</button>`).join('')}</div>`,null,button('Խմբագրել սարքը','device',id,'primary'));paintPorts();}
...
function renderPortTools(){
  ...
  const choices=all.filter(x=>x.d.type==='switch'&&!usedTargets.has(x.p.id)).map(x=>[x.p.id,`${x.f.name} / ${x.r.name} / ${x.d.name} / ${x.p.number}${x.p.status==='fault'?' · ԱՆՍԱՐՔ':''}`]);
  host.innerHTML=`<div class="port-editor-heading"><div><div class="eyebrow">ԸՆՏՐՎԱԾ ՊՈՐՏ</div><h2>${esc(d.name)} / ${p.number}</h2></div>${button('×','port-deselect','','small')}</div><p class="port-title">${esc(r.name)} · ${d.type==='panel'?'Փաչ պանել':'Սվիչ'}</p>
  <form id="portForm">
    <fieldset class="link-config"><legend>Link config</legend>
      ...
      ${input('vlan','VLAN',v.vlan||'','number','min="1" max="4094" step="1" placeholder="Օրինակ՝ 20"')}
      <p class="hint">VLAN 1–4094 · տեղեկատվական դաշտ</p>
    </fieldset>
    ${incoming?`<div class="connection-path">Կապ՝ ${esc(incoming.d.name)} / ${incoming.p.number}<br><small>Նշանակությունը, VLAN-ն ու մալուխի տվյալները ընդհանուր են կապի երկու ծայրերի համար։</small></div>`:''}
    <div class="form-grid">
      ${select('status','Վիճակ',incoming?[['used','Զբաղված'],['fault','Անսարք']]:Object.entries(D.statuses),v.status)}
      ${input('cable','Մալուխի համար',v.cable,'text','maxlength="200" placeholder="C-024"')}
      ${select('floorId','Նպատակակետի հարկ',[['','Ընտրել'],...state.floors.map(f=>[f.id,f.name])],v.floorId)}
      ${input('room','Սենյակ',v.room,'text','maxlength="200" placeholder="205"')}
      ${input('door','Դուռ / տեղադրություն',v.door,'text','maxlength="200" placeholder="Աջ կողմի դուռ"')}
      ${input('side','Կողմ',v.side,'text','maxlength="200" list="sides"')}
      <datalist id="sides"><option value="Աջ"><option value="Ձախ"><option value="Կենտրոն"></datalist>
      ${d.type==='panel'?`<div class="field full">${select('switchPortId','Միացված սվիչի պորտ',[['','Կապ չկա'],...choices],v.switchPortId)}</div>`:''}
    ...
    <div class="form-actions">${button('Չեղարկել','port-discard','','small')}${button('Մաքրել','port-clear',p.id,'danger')}<button type="submit" class="button primary">Պահպանել</button></div>
  </form>`;
  const form=$('#portForm');
  form.addEventListener('input',stagePortDraft);
  form.addEventListener('change',stagePortDraft);
  form.onsubmit=async e=>{e.preventDefault();stagePortDraft();if(flushPortEditor()&&await save())toast('Պորտը պահպանված է');};
}
function stagePortDraft(){
  ...
  if($('#portSaveStatus'))$('#portSaveStatus').textContent='Փոփոխվում է…';
  recovery();
...
function flushPortEditor(){
  ...
  try{
    const form=$('#portForm');if(form&&!form.checkValidity())throw new Error('Ստուգեք դաշտերը։ VLAN-ը պետք է լինի 1–4094 ամբողջ թիվ կամ դատարկ։');
    commit(s=>{
      const entry=D.ports(s).find(x=>x.p.id===id);if(!entry)throw new Error('Պորտը չի գտնվել');
    ...
    if($('#portSaveStatus'))$('#portSaveStatus').textContent='Փոփոխությունը պատրաստ է պահպանման։';
    ...
    if($('#portError')){$('#portError').hidden=false;$('#portError').textContent=e.message;}
    if($('#portSaveStatus'))$('#portSaveStatus').textContent='Չի պահպանվել։ Ուղղեք սխալը կամ չեղարկեք փոփոխությունը։';
    status('Պորտի փոփոխությունը չի պահպանվել',true);return false;
  }
}
function portModal(id){
  if(!flushPortEditor())return;
  const item=findPort(id);if(!item)return;
  selectedPortId=id;portDraft=null;portDraftDirty=false;panelPrefs.right=true;applyPanels();
  $('#dialog').close();
...
async function uploadPhoto(id,file){if(!file)return;if(!['image/png','image/jpeg','image/webp'].includes(file.type))throw new Error('Ընտրեք PNG, JPEG կամ WebP լուսանկար');if(file.size>12*1024*1024)throw new Error('Լուսանկարը պետք է լինի մինչև 12 ՄԲ');
  const uploadCompany=activeCompanyId;
  const bitmap=await createImageBitmap(file);const scale=Math.min(1,1600/Math.max(bitmap.width,bitmap.height)),canvas=document.createElement('canvas');canvas.width=Math.round(bitmap.width*scale);canvas.height=Math.round(bitmap.height*scale);const ctx=canvas.getContext('2d');ctx.fillStyle='#fff';ctx.fillRect(0,0,canvas.width,canvas.height);ctx.drawImage(bitmap,0,0,canvas.width,canvas.height);bitmap.close();const photo=canvas.toDataURL('image/jpeg',.82);if(uploadCompany!==activeCompanyId)throw new Error('Ընկերությունը փոխվել է։ Լուսանկարը նորից ընտրեք համապատասխան ընկերությունում։');commit(s=>{s.floors.flatMap(f=>f.racks).find(r=>r.id===id).photo=photo;});toast('Լուսանկարը ավելացվել է');
}
...
async function restoreFile(file){if(!file)return;if(file.size>24*1024*1024)throw new Error('Ֆայլը չափազանց մեծ է');let data;try{data=JSON.parse(await file.text());}catch{throw new Error('JSON ֆայլը վնասված է');}const next=data.state||data;D.validate(next);
  ...
  confirmAction('Վերականգնել պատճենը',`Ընկերություն՝ ${next.company}։ Հարկեր՝ ${next.floors.length}։ Ներկայիս տվյալները կփոխարինվեն և կպահվեն պատմության մեջ։`,()=>{
...
function deleteItem(kind,id){const item=kind==='floor'?state.floors.find(f=>f.id===id):kind==='rack'?findRack(id)?.r:findDevice(id)?.d;if(!item)return;confirmAction(`Ջնջել ${item.name}`,kind==='floor'?'Կջնջվեն այս հարկի ռաքերը, սարքերը և պորտերը։ Նպատակակետերի հարկի հղումները կմաքրվեն։':'Կջնջվեն նաև ներառված պորտերի տվյալները և դրանց կապերը։ Նախորդ տարբերակը կմնա պահպանման պատմության մեջ։',()=>{
...
async function reloadShared(){if((dirty||portDraftDirty)&&!confirm('Բեռնե՞լ ընդհանուր տարբերակը։ Ձեր չպահված փոփոխությունները պահեք «Ներբեռնել իմ տվյալները» կոճակով։'))return;const data=await api('/api/state');state=data.state;revision=data.revision;dirty=false;conflict=false;clearTimeout(portTimer);portDraftDirty=false;portDraft=null;$('#connectionBanner').hidden=true;$('#dialog').close();status('Պահված է');render();}
function renderCompanySelect(){
  const select=$('#companySelect');if(!select)return;
  select.innerHTML=opts(companies.map(c=>[c.id,c.id===activeCompanyId?(state.company||'Նոր ընկերություն'):(c.name||'Նոր ընկերություն')]),activeCompanyId);
...
async function switchCompany(id){
  ...
  try{
    ...
    $('#dialog').close();$('#connectionBanner').hidden=true;status('Պահված է');render();
    try{if(localStorage.getItem(recoveryKey()))banner('Այս ընկերության համար այս սարքում կա չպահված պատճեն։ Բացեք Կարգավորումները՝ այն վերականգնելու համար։');}catch{}
  }finally{companyBusy=false;$('#content').inert=false;renderCompanySelect();}
}
function newCompany(){
  modal('Նոր ընկերություն',`<div class="form-grid">${input('name','Ընկերության անվանում','','text','required maxlength="200"')}${input('floorCount','Հարկերի քանակ',1,'number','required min="1" max="200"')}</div><p class="hint">Կստեղծվի առանձին ընկերություն։ Գործող ընկերության ռաքերն ու կապերը կմնան իրենց տեղում։</p>`,async fd=>{
    if(!await save())throw new Error('Նախ պահպանեք գործող ընկերության փոփոխությունները։');
...
const actions={
  ...
  save:async()=>{if(await save())toast('Տվյալները պահպանված են');},close:()=>$('#dialog').close(),
  ...
  'port-discard':()=>{clearTimeout(portTimer);portDraftDirty=false;portDraft=null;renderPortTools();status(dirty?'Չպահված փոփոխություններ':'Պահված է');},
  'port-clear':id=>{
    const host=$('#portError');if(!host)return;
    host.hidden=false;host.innerHTML='Մաքրե՞լ պորտի տվյալները և կապը։ '+button('Այո, մաքրել','port-clear-confirm',id,'danger small')+button('Չեղարկել','port-clear-cancel','','small');
  },
  ...
  company:()=>modal('Ընկերություն',input('name','Ընկերության անվանում',state.company,'text','required maxlength="200"'),fd=>commit(s=>{s.company=fd.get('name').trim();if(!s.company)throw new Error('Անվանումը պարտադիր է');})),
  'bulk-floors':()=>modal('Ավելացնել հարկեր',input('count','Քանի նոր հարկ ավելացնել',1,'number',`required min="1" max="${200-state.floors.length}"`),fd=>commit(s=>{let n=s.floors.length+1;for(let i=0;i<+fd.get('count');i++){while(s.floors.some(f=>f.name===`${n}-րդ հարկ`))n++;s.floors.push({id:uid(),name:`${n++}-րդ հարկ`,racks:[]});}})),
  'photo-upload':id=>{const input=$('#photoInput');input.onchange=()=>uploadPhoto(id,input.files[0]).catch(e=>toast(e.message));input.click();},
  photo:id=>modal('Ռաքի լուսանկար',`<img class="photo-full" src="${esc(findRack(id).r.photo)}" alt="Ռաքի լուսանկար">`),
  'photo-remove':id=>confirmAction('Հեռացնել լուսանկարը','Լուսանկարը կհեռացվի այս ռաքից։',()=>commit(s=>s.floors.flatMap(f=>f.racks).find(r=>r.id===id).photo='')),
  ...
  history:async()=>{const rows=await api('/api/history');modal('Պահպանման պատմություն',rows.length?rows.map(x=>`<div class="history-row"><span>Տարբերակ ${x.revision}<br><small>${esc(new Date(x.saved_at).toLocaleString('hy-AM'))}</small></span>${button('Վերականգնել','history-restore',x.revision,'small')}</div>`).join(''):'<p class="hint">Պահպանված տարբերակներ դեռ չկան։</p>');},
  'history-restore':async id=>{const {state:old}=await api('/api/history/'+id);D.validate(old);confirmAction('Վերականգնել նախորդ տարբերակը',`Տարբերակ ${id}։ Ներկայիս տվյալներն ավտոմատ կպահվեն պատմության մեջ։`,()=>commit(s=>{Object.keys(s).forEach(k=>delete s[k]);Object.assign(s,old);}));}
};
document.addEventListener('click',e=>{
  const navigation=e.target.closest('a[href^="#"]');
  const control=e.target.closest('[data-action]');
  if(portDraftDirty&&((navigation)||control&&!['toggle-left','toggle-right','backup','reload','port-discard','port-clear-confirm','port-clear-cancel'].includes(control.dataset.action))){
...
async function init(){
  if(location.protocol==='file:'){
    status('Բացվում է ծրագիրը…');
    $('#content').innerHTML='<section class="panel empty"><h1>Բացել RackMap-ը</h1><p>Ծրագիրը բացվում է տեղական սերվերից։ Եթե ինքնաբերաբար չբացվեց, սեղմեք ներքևի կոճակը։</p><a class="button primary" href="http://localhost:3000/">Բացել ծրագիրը →</a><p class="hint" style="margin-top:20px">Եթե հասցեն հասանելի չէ, նախ գործարկեք այս պանակի Start-RackMap.cmd ֆայլը։</p></section>';
    location.replace('http://localhost:3000/');
    return;
  }
  try{
    const config=await (await fetch('/api/config')).json();cloudMode=config.cloud;maxStateBytes=config.maxStateBytes||maxStateBytes;
    if(cloudMode&&!$('[data-action=logout]'))$('.save-tools').insertAdjacentHTML('beforeend',button('Դուրս գալ','logout','','small'));
    companies=await api('/api/companies');
    if(!companies.some(c=>c.id===activeCompanyId))activeCompanyId=companies[0]?.id||'default';
    const data=await api('/api/state');D.validate(data.state);state=data.state;revision=data.revision;ready=true;status('Պահված է');render();try{if(localStorage.getItem(recoveryKey()))banner('Այս սարքում կա չպահված պատճեն։ Այն կարող եք վերականգնել Կարգավորումներ բաժնից։');}catch{}}catch(e){if(e.code===401&&cloudMode){renderLogin();return;}status('Կապ չկա',true);$('#content').innerHTML=`<section class="panel empty"><h1>Չհաջողվեց միանալ</h1><p>Գործարկեք Start-RackMap.cmd ֆայլը և բացեք http://localhost:3000 հասցեն։</p><p>${esc(e.message)}</p><button class="button primary" data-action="retry">Կրկին փորձել</button></section>`;}
}
actions.retry=init;
