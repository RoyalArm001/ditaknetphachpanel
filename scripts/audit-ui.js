...
(async()=>{
  ...
  async function inspect(page,name,screenshot=false){
    name=await page.locator('html').getAttribute('data-theme')+'-'+name;
  ...
  try{
    for(const colorScheme of ['light','dark'])for(const width of [390,768,1440])for(const lang of ['hy','en','ru']){
      const context=await browser.newContext({viewport:{width,height:940},colorScheme}),page=await context.newPage();page.on('pageerror',e=>errors.push(e.message));
      ...
      await page.click('#themeToggle');assert.equal(await page.locator('html').getAttribute('data-theme'),colorScheme==='dark'?'light':'dark');assert.equal(await page.inputValue('#password'),'unfinished-test-password');await page.click('#themeToggle');
      ...
      await page.reload();assert.equal(await page.locator('html').getAttribute('data-theme'),colorScheme);await page.click('[data-action=welcome-personal]');await page.locator('.stats').waitFor();
