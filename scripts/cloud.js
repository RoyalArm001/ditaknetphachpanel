...
async function main(){
  ...
  try{
    if(command==='migrate'){const exists=await pool.query("SELECT to_regclass('rackmap.schema_version') AS name");let version=0;if(exists.rows[0].name)version=(await pool.query('SELECT max(version) AS version FROM rackmap.schema_version')).rows[0].version||0;if(version>5)throw new Error('Newer cloud schema');for(const [n,file] of [[1,'001-cloud.sql'],[2,'002-pin.sql'],[3,'003-multiple-pins.sql'],[4,'004-personal-accounts.sql'],[5,'005-account-profiles.sql']])if(version<n)await pool.query(fs.readFileSync(path.join(__dirname,'../migrations',file),'utf8'));console.log('Cloud schema ready. Existing company data unchanged.');}
